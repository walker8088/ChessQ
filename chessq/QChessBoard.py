...
class QChessBoard(QWidget):
    ...
    def mousePressEvent(self, mouseEvent):
        ...
        if piece and piece.side == self._board.move_side:
            ...
            if (self.move_side != None) and (new_man.side == self.move_side):
                #选中的是可走方的棋子
                self.last_pickup = (x, y)
                #self.last_move = new_man
            else:
                #以前没选中，现在选中的是对方的棋子，不动作
                if not self.last_pickup:
                    return

                #可能是吃子，交由player自己管理
                #if  self.hook_move[self.move_side] :
                #    self.hook_move[self.move_side](self.last_pickup, (x,y))
                #pub.sendMessage( "side_move_request", from_pos=self.last_pickup, to_pos=(x, y))

        else:
            #空击,如果已经有选过，则可能是走子
            if self.last_pickup:  #and  self.hook_move[self.move_side] :
                #self.hook_move[self.move_side](self.last_pickup, (x,y))
                pass
        '''        
        self.update()
