...
if __name__ == '__main__':
    ...
    books = load_from_epd("..\\chessbooks\\end_games_raw.epd")
    ...
    bad_games = []
    for book in books :
        moves = chanlledge_end_game(book, engine)
        if moves :
                end_game_str = "%02d  %s moves %s" % ( (len(moves)+1)/2, book.strip(), ' '.join(moves))
    ...
    with open("..\\chessbooks\\end_games.epd", "wb") as f:
